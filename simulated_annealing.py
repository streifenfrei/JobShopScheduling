...
def simulated_annealing(problem: JobShopProblem, max_time = 4000, r = 0.00005, t_max = 100000, t_min = 1):
    ...
    while t >= t_min and time.time() - start_time <= max_time:
        ...
        while opt_count <= 250:
            ...
            print(neighbour.get_length())
            print(sol.get_length())
            print(delta)
            if delta <= 0:
                sol = neighbour.copy()
                neighbours = sol._random_neighbour_generator()                  
                ...
                print("d: ", delta)
                print("t: ", t)
                print("p: ",calc_probability(delta, t))
                print(random.random())
                sol = neighbour.copy()
                neighbours = sol._random_neighbour_generator() 
                num_pro += 1             
...
def simulated_annealing_three(problem: JobShopProblem, max_time = 4000, r = 0.00005, t_max = 100000000, t_min = 1):
    start_time = time.time()
    initial_solution = Schedule.create_from_problem(problem)
    best_solution = initial_solution.copy()
    j = 0
...
def simulated_annealing_four(problem: JobShopProblem, max_time = 4000, r = 0.00005, t_max = 100000000, t_min = 1):
    start_time = time.time()
    initial_solution = Schedule.create_from_problem(problem)
    best_solution = initial_solution.copy()
    j = 0
